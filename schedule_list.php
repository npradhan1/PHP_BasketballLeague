<?php
	if(isset($_GET['gamemonth'])) {
			session_start(); 
			$_SESSION['gamemonth'] = $_GET['gamemonth'];
	}
	// connect to database
	require 'connect.php';
	...
	while($row = mysql_fetch_array($schedule)) {
				...
				// check month
				if ($_SESSION['gamemonth'] == $gamemonth[$num]){
					...
					$fields = mysql_query("SELECT * FROM division WHERE dcode='{$dcode[$num]}'") or die("Unable to connect to query");
...
<tr>
   ...
   <?php
		echo ($usercheck == "true" ? "You are logged in as '{$_SESSION['userID']}' &nbsp; <a href=\"admin.php\">Admin<a> &nbsp; <a href=\"logout.php\">Logout<a>" : "<p>Login to register for the basketball 2012-2013 season. <a href=\"login.php\">Login</a> &emsp; <a href=\"sitemap.php\">Sitemap</a> </p>"); 
	?> 
	</td>
	</tr>
<tr><td>&nbsp;</td>
...
<?php
	...
	if(count($data)>0){
		...
		echo "<tr height=\"25\"><td><div align=\"center\">No Game</div></td>
