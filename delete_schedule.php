<?php
	...
	while($row = mysql_fetch_array($schedule)) {
		...
		if($usertype == 2){
			if($divisionCode == $row['dcode']){
					...
					$data[$num] = array('dcode' => $dcode[$num],'gid' => $gid[$num], 'gamedate' => $gamedate[$num], 'gametime' => $gametime[$num], 'gym' => $gym[$num], 'awayteam'=> $awayteam[$num], 'hometeam'=> $hometeam[$num]
	...
	foreach ($data as $key => $row) {
				...
				$radiovalue[$key] = $dcode[$key]. "/" .$gid[$key];
				
				//&&gid=$gid[$key] dcode=$_SESSION['game_division']
				$show_list[$key]= "<tr height=\"25\">
				<td><div align=\"center\">
				<input type=\"radio\" name=\"delete\" id=\"delete\" value=$radiovalue[$key]>
				</td>
				<td><div align=\"center\">$gamedate[$key]</div></td>
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if($checked == false){
                $error_arr['schedule'] = 'Please select a button';
        }
		else{
				$radiovalue = $_POST['delete'];
		}
		
		if(count($error_arr) == 0){
			list($dcode, $gid) = split('/', $radiovalue);
	
			mysql_query("DELETE FROM schedule where (dcode='$dcode')&&(gid=$gid)");
...
<tr height=30>
<td><align="left" style="color:red; font-size: 12px;"/><?php echo "Please select a schedule";?> &nbsp;</td></tr></table>
<table align=center width="500">
...
<table align=center width="500">
<tr><td>
<align="left" style="color:red; font-size: 12px;"/>&nbsp;<?php echo $error_arr['schedule'];?>
</td></tr>
</table>
</body>
</html>